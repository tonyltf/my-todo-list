...
export class TodoController {
    ...
    async getTodo(
        ...
        reply
            .status(200)
            .send(allTodo.map((todo) => TodoController.transformTodo(todo)));
    }
    ...
    async createTodo(
        ...
        const newTodo = await this.todoService.createTodoForUser({
            userId,
            todoData,
        });
        if (!newTodo) {
    ...
    async updateTodo(
        ...
        const existingTodo = await this.todoService.getTodoById({
            userId,
            todoId,
        });
        if (!existingTodo) {
    ...
    async deleteTodo(
        request: FastifyRequest<{ Params: { userId: string; todoId: string } }>,
        reply: FastifyReply,
    ) {
        const { userId, todoId } = request.params;
