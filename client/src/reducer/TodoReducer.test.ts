...
describe('reducer', () => {
    ...
    it('should handle SET_INITIAL_TODOS action', () => {
        ...
        const todos = [{ id: 'todo-1', name: 'Task 1', userId: 'user-id-1', isCompleted: false, isEnabled: true, completedAt: null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }];
        const action = { type: TodoAction.SET_INITIAL_TODOS, payload: todos } as Action;
        const newState = reducer(initialState, action);
    ...
    it('should handle ADD_TODO action', () => {
        const initialState = { isLoading: false, error: null, todos: [] };
        const todo = { id: 'todo-1', name: 'Task 1', userId: 'user-id-1', isCompleted: false, isEnabled: true, completedAt: null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
        const action = { type: TodoAction.ADD_TODO, payload: todo } as Action;
        const newState = reducer(initialState, action);
        expect(newState).toEqual({ isLoading: false, error: null, todos: [todo] });
    });
